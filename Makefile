...
pkgdatadir = $(datadir)/econocalypse
pkgincludedir = $(includedir)/econocalypse
pkglibdir = $(libdir)/econocalypse
pkglibexecdir = $(libexecdir)/econocalypse
...
CONFIG_HEADER = $(top_builddir)/src/config/econocalypse-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/econocalypse-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/Econocalypse/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /tmp/Econocalypse/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/Econocalypse/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/Econocalypse/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = econocalypse-cli
BITCOIN_DAEMON_NAME = econocalypsed
BITCOIN_GUI_NAME = econocalypse-qt
BITCOIN_TX_NAME = econocalypse-tx
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/Econocalypse/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = econocalypse
PACKAGE_BUGREPORT = www.econocalypse.org
PACKAGE_NAME = Econocalypse Core
PACKAGE_STRING = Econocalypse Core 1.0.0
PACKAGE_TARNAME = econocalypse
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/Econocalypse
abs_srcdir = /tmp/Econocalypse
abs_top_builddir = /tmp/Econocalypse
abs_top_srcdir = /tmp/Econocalypse
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/Econocalypse/build-aux/install-sh
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_econocalypse_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_econocalypse_coverage.info test_econocalypse.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Econocalypse-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/econocalypse-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/econocalypse-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/econocalypse-config.h
$(top_srcdir)/src/config/econocalypse-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/econocalypse-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/Econocalypse-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Econocalypse-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Econocalypse-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_econocalypse.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_econocalypse -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_econocalypse_filtered.info: test_econocalypse.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_econocalypse_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_econocalypse_coverage.info: baseline_filtered_combined.info test_econocalypse_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_econocalypse_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_econocalypse_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_econocalypse_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_econocalypse.coverage/.dirstamp:  test_econocalypse_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_econocalypse.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_econocalypse.coverage/ total.coverage/ $(OSX_APP)
