...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SLACK);
    ...
    if (!settings.contains("nAnonymizeEconocalypseAmount"))
        settings.setValue("nAnonymizeEconocalypseAmount", 1000);

    nAnonymizeEconocalypseAmount = settings.value("nAnonymizeEconocalypseAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeEconocalypseAmount"))
        SoftSetArg("-anonymizeeconocalypseamount", settings.value("nAnonymizeEconocalypseAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in econocalypse.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeEconocalypseAmount:
            return QVariant(nAnonymizeEconocalypseAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeEconocalypseAmount:
            nAnonymizeEconocalypseAmount = value.toInt();
            settings.setValue("nAnonymizeEconocalypseAmount", nAnonymizeEconocalypseAmount);
            emit anonymizeEconocalypseAmountChanged(nAnonymizeEconocalypseAmount);
            break;
        case CoinControlFeatures:
