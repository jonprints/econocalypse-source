...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zslackTracker->HasMintTx(hash))) {
            //zSLACK stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZSLACK;
        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by Econocalypse Address
...
bool IsZSLACKType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZSLACK:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZSLACK || type == TransactionRecord::MNReward) {
