...
CzSLACKWallet::CzSLACKWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zslack seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSLACKSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSLACKSeed_deprecated()) {
                    LogPrintf("%s: Updated zSLACK seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zslack seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zslack wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSLACKWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSLACKCount(nCountLastUsed);
    else if (!walletdb.ReadZSLACKCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSLACKWallet::Lock()
{
    seedMaster = 0;
}

void CzSLACKWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSLACKWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSLACK(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSLACKWallet::LoadMintPoolFromDB()
{
...
void CzSLACKWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSLACKWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSLACKWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zslackTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSLACKWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSLACK(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zslackTracker which also adds to database
    pwalletMain->zslackTracker->Add(dMint, true);
...
void CzSLACKWallet::SeedToZSLACK(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSLACKWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSLACKWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZSLACKCount(nCountLastUsed);
...
void CzSLACKWallet::GenerateDeterministicZSLACK(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSLACKWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSLACKWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
