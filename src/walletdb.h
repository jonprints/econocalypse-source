...
public:
    ...
    bool WriteZSLACKSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSLACKSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSLACKSeed_deprecated(uint256& seed);
    bool EraseZSLACKSeed();
    bool EraseZSLACKSeed_deprecated();

    bool WriteZSLACKCount(const uint32_t& nCount);
    bool ReadZSLACKCount(uint32_t& nCount);
