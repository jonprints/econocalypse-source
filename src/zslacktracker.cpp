...
CzSLACKTracker::CzSLACKTracker(std::string strWalletFile)
{
...
CzSLACKTracker::~CzSLACKTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSLACKTracker::Init()
{
...
bool CzSLACKTracker::Archive(CMintMeta& meta)
{
...
bool CzSLACKTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSLACKTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSLACKTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSLACKTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzSLACKTracker::GetSerialHashes()
{
...
CAmount CzSLACKTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSLACKTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSLACKTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSLACKTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSLACKTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSLACKTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSLACKTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSLACKTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSLACKTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSLACKTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSLACKTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSLACKTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSLACKTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSLACKTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSLACKTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSLACKTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSLACKTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzslack from DB\n", __func__, listDeterministicDB.size());
    }
