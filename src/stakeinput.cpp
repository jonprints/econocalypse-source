...
// The zSLACK block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zSLACK is a hash of the serial
...
{
    //Create an output returning the zSLACK that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZSLACKOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zSLACK output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zSLACK", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZSLACKOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zSLACK output", __func__);
...
{
    CzSLACKTracker* zslackTracker = pwallet->zslackTracker.get();
    CMintMeta meta;
    if (!zslackTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zslackTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a SLACK stake is the outpoint
